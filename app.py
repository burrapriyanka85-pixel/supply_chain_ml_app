...
# Initialize DB on run (Local only) - PROTECTED
if not IS_CLOUD:
    init_db()
else:
...
# FIX 4: NEVER AUTO-LOAD LOCAL MODEL FILES IF ON CLOUD
if "last_model" not in st.session_state:
    if not IS_CLOUD:
        ...
        st.session_state["last_model_err"] = "Upload a model to begin"
...
with st.sidebar:
    ...
    if st.session_state.get("current_user"):
        ...
        # ----- AUTH PAGES -----
        # CLOUD-SAFE REPLACEMENT
        if IS_CLOUD:
            st.info("🔒 Authentication is disabled on Streamlit Cloud demo.")
            st.session_state["page"] = "Home"
        else:
            auth_page = st.radio(
                "Account",
